...
function Toggle({ label, enabled, onToggle }: { label: string; enabled: boolean; onToggle: () => void }) {
  return (
    <div className="flex items-center justify-between px-5 py-4 border-b border-stone-800">
      <span className="text-[10px] uppercase tracking-widest font-extrabold text-white">{label}</span>
      <button
        onClick={onToggle}
...
export function GameplayModal({ onClose, classicMode, onClassicMode }: Props) {
  return (
    <div
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-700">
          <div className="text-amber-400 font-bold text-base">Gameplay</div>
